...
class Product(db.Model):
    ...
    # ── image_url stores ONLY the bare filename, e.g. "black-jersey.jpg"
    # ── NEVER store "static/uploads/black-jersey.jpg" here
    # ── Templates use: url_for('static', filename='uploads/' + product.image_url)
    image_url   = db.Column(db.String(500), nullable=True)

    # One product → many images (cascade delete keeps disk clean)
    ...
    def size_list(self):
        """Return sizes as a Python list. e.g. ['S', 'M', 'L', 'XL']"""
        if self.sizes:
    ...
    def get_image_url(self):
        """
        Safe helper to call in templates if you want:
            {{ product.get_image_url() }}
        Returns the full static path string or empty string.
        """
        if self.image_url:
...
class ProductImage(db.Model):
    ...
    id         = db.Column(db.Integer, primary_key=True)
    # url stores ONLY the bare filename, same rule as Product.image_url
...
class Order(db.Model):
    ...
    def get_whatsapp_items(self):
        try:
            ...
            return "\n".join([f"• {item['name']} ({item.get('size', 'One Size')})" for item in data])
        except Exception:
